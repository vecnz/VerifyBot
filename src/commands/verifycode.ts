import {
  addVerifiedRoleToUser,
  informUserOfError,
  removeVerifiedRoleFromUser,
} from "#lib/utils";
        ...
        builder //
          ...
          .addStringOption((input) =>
            input //
              .setName("code")
              .setDescription(
                "The code that was emailed to you when you ran the /verify command."
              )
              .setRequired(true)
          )
      );
  ...
  public override async chatInputRun(
    ...
    if (!user || !verificationRecord) {
      await interaction.editReply({
        content:
          "You have not started verification. Please run the /verify command.",
      });
      return;
    }
    ...
    if (verificationRecord.completed) {
      ...
      await interaction.editReply({
        content:
          "You have already completed verification. If you wish to re-verify please run `/verify`",
      });
      return;
    }
    ...
    if (otherUser) {
      try {
        ...
        await informUserOfError(
          interaction,
          error,
          "removing the verified status from the user previously verified with this email"
        );
        return;
      }
    }
    ...
    try {
      ...
      await informUserOfError(
        interaction,
        error,
        "adding the verified role to you"
      );
    }
